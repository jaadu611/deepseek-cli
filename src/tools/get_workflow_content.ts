...
module.exports = {
  ...
  description: 'Retrieves the full content of a workflow file by its ID (filename without .md extension). Searches in project-specific directories (./workflows, ./.workflows, ./.github/workflows, ./ds_config/workflows) first, then in the global ~/.deepseek_cli/workflows/ directory.',
  parameters: {
    type: 'object',
    properties: {
      workflow_id: {
  ...
  async execute({ workflow_id }) {
    ...
    const globalWorkflowsDir = path.join(os.homedir(), '.deepseek_cli', 'workflows');
